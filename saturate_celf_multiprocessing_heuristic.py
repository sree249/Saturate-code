...
#the adv_oracle works as follows:
'''
...
def adv_oracle(budget,adj,key_list, cur_set):
	...
	#add budget constraint - total no. of monitors === K
	...
	for i in range(0,len(key_list)):
		if key_list[i] in adj:
			...
			if len(common_ngh) > 0:
				...
				m.addConstr(y[i] >= gp.quicksum(x_list)/total_nodes)
			else:
	...
	while obj_val < c and not(exit_flag): #and comp_val > 0:
		...
		if len(new_list) < len(invitees_list_cur): #check that  there are elements possible to be added
			...
			if max_flag == 1: #this is the 1st pass
				print('First pass results from sim no.' ,sim_no)
				...
				if next_top[0] > marginal_value:
					print('not to recompute, current readdition is',cur_top[1],'in simulation no.', sim_no)
			...
			print('current A list length ',len(a),' cur a is ',a,'in simulation no.', sim_no)
			#print('comp_val currently',comp_val)
			print('func val in GPC',obj_val,'in sim no.',sim_no)
		else:
			exit_flag == 1
...
def run_simulation(adj, key_list,type,sim_no,alpha,k):

	#open_file to write details of simulation
	#construct file_name 
	file_name = "sim_results_"+str(type)+"_"+str(sim_no)+"_"+str(k)+"_"+str(alpha)+".txt"
	...
	if num_sets < total_sets:
		...
		print('non overlapping subsets',num_sets,'in sim no',sim_no)
	...
	while num_trainings < num_training_sessions and len(total_nodes_covered) != len(key_list):
		print('------------------------------------------------------')
		print('result for stage: ',num_trainings+1,'in sim no', sim_no)
		...
		if len(cur_invitees) > k:
			...
			while c_max - c_min >= 0.5 and not(binary_flag):
				print('In sim no',sim_no,'diff',c_max-c_min)
			
				c = (c_max+c_min)/2
				print('In sim no',sim_no,'current c',c)
		
				cur_seed = GPC(cur_invitees,cur_adj,cur_uncertainty_sets,c,prev_covered,sim_no)
				print('In sim no', sim_no,'cur_seed ',cur_seed,' size ',len(cur_seed))
				
				#generate a new subset
				budget = int(alpha*len(cur_seed))
				if len(cur_uncertainty_sets) < NUM_TOTAL_SETS: #and len(cur_seed) > int(beta*k):
					output = adv_oracle(budget,cur_adj,cur_keylist,cur_seed)
					new_uncertainty_set = output[0]
					cur_uncertainty_sets.append(new_uncertainty_set)
			...
			print('In sim no',sim_no,'best seed for stage: ',num_trainings+1,' is:',best_seed,' with length ',len(best_seed))
		...
		file_handle.write(str(best_seed))
		file_handle.write("\n")
		file_handle.write(str(len(best_seed)))
		file_handle.write("\n")
		...
		print('In sim no',sim_no,'finally based on random simulation attendees are ',final_seed)
		...
		if num_trainings + 1 < num_training_sessions and len(total_nodes_covered)< len(key_list):
			...
			print('In sim no',sim_no,'new adj ',new_adj)
		...
		print('---------------------------------------------------------------------------')
		file_handle.write("\n")
	file_handle.write(str(len(total_nodes_covered)))
	file_handle.write("\n")
	file_handle.write(str(stage_wise_coverage))
	end = t.time()

	diff = end - start
	print('hours elapsed:for sim',sim_no,'is', diff/3600)
	file_handle.write("\n")
	file_handle.write('hours elapsed:for sim '+str(sim_no)+' is '+str(diff/3600))
	file.close()
...
if __name__ == '__main__':	
	...
	for sim in range(0,num_iterations):
		...
		p = Process(target = run_simulation, args = (adj, key_list,type_num,sim,alpha,k,))
		p.start()
		#p.join()
