...
k = 8 #budget of how many invitees per round
alpha = 0.5
num_noshows = int((1-alpha)*k) #changeable
total_sets = 50 #changable
num_iterations = 4
...
file = open("sim_graph_50_med.txt","r")
...
while num_trainings < num_training_sessions and len(total_nodes_covered) != len(key_list):
	...
	stage_len_best_seed.append(len(best_seed))
		
	budget = int(alpha*len(best_seed))
	worst_case = adv_oracle(budget,cur_adj,cur_keylist,best_seed)
...
print('length of best seeds in each stage', stage_len_best_seed)
