...
def run_simulation(adj, key_list,type,sim_no):

	#open_file to write details of simulation
	#construct file_name 
	file_name = "sim_results_"+str(type)+"_"+str(sim_no)+".txt"
...
for sim in range(0,num_iterations):
	...
	th = threading.Thread(target = run_simulation, args = (adj, key_list,type_num,sim,))
	threads.append(th)
	th.start()
		#p.join()
