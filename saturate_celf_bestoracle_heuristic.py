...
beta = .4 #beta is the % for new set generation
...
'''
def adv_oracle(budget,adj,key_list, cur_set,alpha):
...
for sim in range(0,num_iterations):
	...
	while num_trainings < num_training_sessions and len(total_nodes_covered) != len(key_list):
		...
		if len(cur_invitees) > k:
			...
			c_min = 0
			c_max =  len(cur_invitees)//2 # the best thing to do is to invite everyone in the network. 
			best_seed = []
			binary_flag = False
