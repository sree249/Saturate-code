...
k = 20 #budget of how many invitees per round
alpha = 0.7
...
for sim in range(0,20):
	...
	num_sets = 20 #len(cur_invitees)//num_noshows + 1 
	
	
	cur_uncertainty_sets = make_subsets(cur_invitees,num_sets,num_noshows) # initialize the uncertainty sets
	...
	while num_trainings < 4 and len(total_nodes_covered) != len(key_list):
		...
		if num_trainings + 1 < 4 and len(total_nodes_covered)< len(key_list):
			...
			cur_uncertainty_sets = make_subsets(cur_invitees,num_new_sets,num_noshows)
