...
def run_simulation(adj,key_list,type,sim_no,alpha):
	...
	#construct file_name 
	file_name = "/auto/rcf-40/subhasrs/multiple_graphs_exps/iterative_saturate_200_25_0.5/sim_results_"+str(type)+"_"+str(alpha)+"_"+str(sim_no)+".txt"
	...
	while num_trainings < num_training_sessions and len(total_nodes_covered) != len(key_list):
		...
		if len(cur_invitees) > k:
			...
			while(len(cur_uncertainty_sets) < num_scenarios):
...
if __name__ == '__main__':	
	...
	for sim in range(0,num_sims):
		...
		filename = '/auto/rcf-40/subhasrs/multiple_graphs_exps/iterative_saturate_200_25_0.5/sim_graph_200_'+str(sim)+'.txt'
...
file_name = "netstats_"+str(k)+"_"+str(alpha)+".txt"
stat_file = open(file_name,"w")
stat_file.write(str(net_stats))
