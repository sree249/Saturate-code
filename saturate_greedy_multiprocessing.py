...
k = 30 #budget of how many invitees per round
alpha = 0.7
...
def GPC(invitees_list_cur,adj_cur,sets,c,pre_covered,sim_no,ipc_q):
	a = []
...
def run_simulation(adj, key_list,type,sim_no,alpha):
	...
	#construct file_name 
	file_name = "sim_results_"+str(type)+"_"+str(sim_no)+"_"+str(alpha)+".txt"
	...
	while num_trainings < num_training_sessions and len(total_nodes_covered) != len(key_list):
		...
		if len(cur_invitees) > k:
			...
			while c_max - c_min >= 0.5 and not(binary_flag):
				...
				#Define a queue for IPC
				ipc_q = Queue()
				
				p = Process(target = GPC, args=(cur_invitees,cur_adj,cur_uncertainty_sets,c,prev_covered,sim_no,ipc_q,))
...
if __name__ == '__main__':	
	...
	for sim in range(0,num_iterations):
		...
		p = Process(target = run_simulation, args = (adj, key_list,type_num,sim,alpha,))
		p.start()
		#p.join()
